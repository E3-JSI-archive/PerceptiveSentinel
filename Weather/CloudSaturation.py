...
class CloudSaturation:
    ...
    def load_all_bands(self):
        ...
        print(all_bands_request.layer)
        
        all_bands_data = all_bands_request.get_data(save_data=True,
                                                    redownload=self.redownload)
        print ('Bands')
        
        self.memo_data = CloudSaturation.MemoData(np.zeros(1),
                                                  np.zeros_like(all_bands_data),
                                                  np.zeros(1), all_bands_data,
                                                  true_color_request.get_dates(), np.zeros(1))
        
        return all_bands_data, true_color_request.get_dates()    
    ...
    def get_tulip_mask(self, layer_name):
        ...
        coord = [self.coordinates[1], self.coordinates[0], self.coordinates[3], self.coordinates[2]]
        tulipFields = TulipFieldRequest(bbox=to_epsg3857(coord),
